...
func _process(delta):
	if Input.is_action_just_pressed("ui_cancel"):
		if !exit_open_close:
			$exit.visible = true
			exit_open_close = true
		else:
			$exit.visible = false
			exit_open_close = false
	
	
	screen_size = get_viewport().size
	
	$margin/year/time.text = str(Global.year)
	$margin/day/time.text = str(Global.day)
	...
	if viewport_son.get_child_count() == 0:
		$menu_planets/open_close/PanelContainer/celestialBody/go.visible = false
	else:
		$menu_planets/open_close/PanelContainer/celestialBody/go.visible = true
	...
	# centra il menu di chiusura
	$exit.position.x = screen_size.x/2
	$exit.position.y = screen_size.y/2
...
func _on_sun_pressed():
	...
	object_mass.text = "1,989 × 1e30 kg"
...
func _on_throw_pressed():
	...
	# ferma lo slider su 1 sec 2 days
	$time.value = 5
	$time.editable = false
