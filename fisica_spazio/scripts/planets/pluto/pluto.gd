...
func _ready():
	...
	orbital_verse = -1
	a = 1973 # Semi-asse maggiore (distanza media dal baricentro)
	...
	perigee_argument = deg_to_rad(180) # variazione in radianti del nodo longitudinale ascendente in un unità di tempo (un secondo della simulazione) (variazione in un periodo orbitale/periodo orbitale)
	...
	time_elapsed =  7.8 * 1e9 # tempo di inizio dell'orbita
	...
	ascension_rect = deg_to_rad(132.993) # inclinazione in radianti asse corpo celeste
